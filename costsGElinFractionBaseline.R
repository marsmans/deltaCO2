...
if (dir.exists("~/disks/y/ontwapps/Timer/Users/Stijn/Model/deltaCO2")) {     #Den Haag
  setwd("~/disks/y/ontwapps/Timer/Users/Stijn/Model/deltaCO2")
} else if (dir.exists("~/Documenten/Stage PBL/deltaCO2")) {    #thuis
  setwd("~/Documenten/Stage PBL/deltaCO2")
}
...
#
# Andere methode: 
# Tabel 6.2 van AR5 WG3 Ch6: verschillende waarden voor deltaCO2 voor concentratie CO2eq
# kostenfiguur: concentratieCO2 op verschillende fracties
...
newHerparam <- read.csv(file = "./../Databases/new_herparam.txt", header = TRUE)
herparametrisering <- lm(data=newHerparam, deltaCO2~fractionBL)


herparam.slope <- coef(herparametrisering)[2]
herparam.intercept <- coef(herparametrisering)[1]
...
f.costsresult <- function(N,cumuCO2result, costs.sample) {
  
  # run model
  costs <- mapply(costs.oneRun, cumuCO2result, costs.sample[,1], costs.sample[,2])
  
  # plak resultaat aan sample
  return(data.frame(costs.sample, costs))
}
